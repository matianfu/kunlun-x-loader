...
#define TOSHIBA_MFR     0x98
#define TOSHIBA_ID      0xBC
#define HYNIX_MFR       0xAD
#define HYNIX4G_ID        0xBC
#define HYNIX8G_ID        0xB3
...
int nand_chip()
{
	...
	if ((mfr == MT29F1G_MFR && (id == MT29F1G_ID ||
		...
		(mfr == TOSHIBA_MFR && (id == TOSHIBA_ID)) ||       // support k29c toshiba nand
		(mfr == HYNIX_MFR && (id == HYNIX4G_ID ||id == HYNIX8G_ID)) ||       // support jingtian hynix nand
...
static int nand_read_page(u_char *buf, ulong page_addr)
{
	...
	for (count = 0; count < ECC_SIZE; count += ECC_STEPS) {
		...
		NAND_ECC_CALC((const u_char *)(p - ECC_BLOCK_SIZE), &ecc_calc[count]);
	}
