...
#ifdef CFG_DO_DDR_TEST
/****************************************
 * Routing: ddr_memory_test(void)
 * Description: Test the Memory, should be called after serial_init
 * *************************************/
void ddr_memory_test(void)
{
    unsigned int i = 0;
    const unsigned int c_value = 0x28828015;
    ...
    for(i = 0x80000000; i < (0x80000000 + DDR_MEMORY_SIZE); i = i + 4)
    {
        *((volatile unsigned int *)i) = c_value;
        temp = *((volatile unsigned int *)i);
        if(temp != c_value)
        {
#ifdef CFG_PRINTF
            printf("\nMemory Error at Address 0x%x\n", i);
            return;
#endif
        }
        if((i % 0x100000) == 0)
    ...
    if(i >= (0x80000000 + DDR_MEMORY_SIZE))
    {
#ifdef CFG_PRINTF
        printf("\nMemory test OK!, Size is 0x%x\n", (i - 0x80000000));
#endif
    }
...
void config_3430sdram_ddr(void)
{
    ...
    /* Change the driven strength to weak, because pop stack */
	__raw_writel((1 << 5), SDRC_EMR_0);
	...
	/* Configure cs1 to be just behind cs0 - 128meg boundary */
		...
		defined(CONFIG_3630ZOOM3) || defined(CONFIG_3630SDP) || defined(CONFIG_3630KUNLUN_WUDANG) || defined(CONFIG_3630KUNLUN_N710E_REV2)
	/* 2 * 128M = 256M for cs1 */
